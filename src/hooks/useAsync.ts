...
const DefaultOptions: UseAsyncOptionsNormalized<any> = {
  initialState: (options) =>
    options && options.executeOnMount ? InitialAsyncLoadingState : InitialAsyncState,
  executeOnMount: true,
...
export const useAsyncAbortable = <R = UnknownResult, Args extends any[] = UnknownArgs>(
  ...
  const asyncFunctionWrapper: (...args: Args) => Promise<R> = async (...p: Args) => {
